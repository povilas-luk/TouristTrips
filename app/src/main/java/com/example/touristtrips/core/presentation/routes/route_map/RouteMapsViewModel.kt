        ...
        viewModelScope.launch {
            ...
            for (i in locations.indices) {
                    if (i < locations.size-1) {
                        val fromLatLng = LatLng(locations[i].latitude.toDouble(), locations[i].longitude.toDouble())
                        val toLatLng = LatLng(locations[i+1].latitude.toDouble(), locations[i+1].longitude.toDouble())
                        when (val directionsResponse = repository.getDirections(fromLatLng, toLatLng)) {
                            is DirectionsResponseResource.Success -> {
                                val shape = directionsResponse.data?.routes?.get(0)?.overviewPolyline?.points
            ...
            /*val fromLatLng = LatLng(locations[0].latitude.toDouble(), locations[0].longitude.toDouble())
            val toLatLng = LatLng(locations[1].latitude.toDouble(), locations[1].longitude.toDouble())
            val directionsResponse = repository.getDirections(fromLatLng, toLatLng)
            val shape = directionsResponse.data?.routes?.get(0)?.overviewPolyline?.points
            val polyline = PolylineOptions()
                .addAll(PolyUtil.decode(shape))
                .width(8f)
